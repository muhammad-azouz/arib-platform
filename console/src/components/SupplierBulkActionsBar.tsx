...
import { useBulkUpdateSuppliers, useCustomerGroups } from '@/lib/hooks'
...
/**
 * Bulk group-assign / pricing-tier toolbar for the selected supplier rows
 * (mirrors BulkActionsBar.tsx — `PUT .../suppliers/bulk`, all-or-nothing on
 * the gateway side). Each action applies independently — picking a group
 * doesn't require also setting a price tier, and vice versa.
 */
export function SupplierBulkActionsBar({
  tenantId,
  selectedIds,
  onClear,
}: {
  ...
  const bulk = useBulkUpdateSuppliers(tenantId)
  ...
  const applyGroup = async () => {
    ...
    try {
      ...
      toast.success(`تم تعيين المجموعة لـ ${toArabicDigits(result.updated)} مورد`)
    } catch (err) {
  ...
  const applyPriceTier = async () => {
    ...
    try {
      ...
      toast.success(`تم تحديث فئة السعر لـ ${toArabicDigits(result.updated)} مورد`)
    } catch (err) {
      toast.error(errorMessage(err))
    }
  }
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-3 rounded-xl border border-primary/30 bg-primary/5 px-4 py-3">
      <span className="text-sm font-medium">
        {toArabicDigits(selectedIds.length)} مورد مُحدد
      </span>
