...
import { AddIcon, ArrowLeading, DownloadIcon, SearchIcon, UsersIcon } from '@/components/icon'
...
function ListView({
  ...
  const [debt, setDebt] = useState<CustomerDebtFilter | undefined>(undefined)
  ...
  const query = useCustomers(tenantId, {
  ...
  const handleExport = async () => {
    ...
    setExporting(true)
    try {
      const blob = await api.exportCustomers(tenantId, {
        search: debouncedSearch || undefined,
        branchId,
        groupId,
        active,
    ...
    <div>
      <div className="mb-4 grid gap-3 sm:grid-cols-[1fr_auto_auto_auto_auto]">
        ...
        <select
          ...
          value={debt ?? ''}
          onChange={(e) => setDebt((e.target.value || undefined) as CustomerDebtFilter | undefined)}
        >
          {DEBT_OPTIONS.map((o) => (
      ...
      {notSubscribed ? (
        <EmptyState
          ...
          description="فعّل اشتراك المزامنة لعرض عملاء فروعك."
        />
      ) : gatewayError ? (
        <ErrorState
          message="تعذّر الوصول إلى بيانات العملاء الآن."
        ...
        <>
          ...
          <CustomersTable
            ...
            onRowClick={(id) => navigate(`/tenants/${tenantId}/customers/${id}`)}
          />
  ...
  if (!items || items.length === 0) {
    return (
      <EmptyState
        icon={UsersIcon}
        title="لا يوجد عملاء"
        description="لا يوجد عملاء مطابقون لبحثك أو للفلاتر المحددة."
      />
    )
  }
...
/** New-customers-per-day bars — same CSS-flex-bar construction as Reports'
      ...
      <div className="flex h-40 items-end gap-px border-b border-border sm:gap-0.5">
        {days.map((d) => (
          <div
            ...
            title={`${format(parseDay(d.day), 'EEEE d MMM', { locale: ar })} — ${toArabicDigits(d.new_customers)} عميل جديد`}
          >
            {d === peak && (
  ...
  return (
    <ul className="space-y-1.5">
      {rows.map((r, i) => (
        <li key={r.id}>
          <Link
            to={`/tenants/${tenantId}/customers/${r.id}`}
      ...
      {items.length === 0 ? (
        ...
        <ul className="space-y-1">
          {items.map((c) => (
            <li key={c.id}>
              <Link
                to={`/tenants/${tenantId}/customers/${c.id}`}
...
function CreditWarningsList({
  ...
  if (rows.length === 0) {
    return <p className="text-sm text-muted-foreground">لا يوجد عملاء يقتربون من حدهم الائتماني.</p>
  }
  return (
    <ul className="space-y-1.5">
      {rows.map((r) => (
...
function InsightsView({
  ...
  const query = useCustomerInsights(tenantId, { branchId, from, to })
    ...
    <div>
      ...
      {notSubscribed ? (
        <EmptyState
          icon={UsersIcon}
          title="لا يوجد اشتراك مزامنة"
          description="فعّل اشتراك المزامنة لعرض تحليلات العملاء."
        />
      ) : gatewayError ? (
        <ErrorState
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <InsightCard title="نمو العملاء الجدد">
            <GrowthChart days={query.data.data.growth_over_time} />
          </InsightCard>

          <InsightCard title="عملاء جدد هذا الشهر">
            <CustomerRefsList
              tenantId={tenantId}
              count={query.data.data.new_this_month.count}
              items={query.data.data.new_this_month.items}
              emptyLabel="لا يوجد عملاء جدد هذا الشهر."
            />
          </InsightCard>

          <InsightCard title="أعلى العملاء (الفترة المحددة)">
            <RankedCustomersList
              tenantId={tenantId}
              rows={query.data.data.top_customers}
              emptyLabel="لا توجد مبيعات لعملاء في هذه الفترة."
            />
          </InsightCard>

          <InsightCard title="الأعلى إنفاقًا (كل الأوقات)">
            <RankedCustomersList
              tenantId={tenantId}
              rows={query.data.data.highest_spenders}
              emptyLabel="لا توجد بيانات إنفاق بعد."
            />
          </InsightCard>

          <InsightCard title="عملاء غير نشطين">
            <CustomerRefsList
              tenantId={tenantId}
              count={query.data.data.inactive.count}
              items={query.data.data.inactive.items}
              emptyLabel="كل العملاء نشطون."
            />
          </InsightCard>
